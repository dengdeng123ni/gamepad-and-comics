...
`window._gh_menu_register` 添加右键菜单
`window._gh_comics_get_detail` 获取漫画数据
`window._gh_comics_get_pages` 获取页面数据
`window._gh_comics_get_image` 获取图像数据 

## 接口 - `window._gh_menu_register(region, Array<{name:string,id:string,click:Function}>)`
## 接口 - `window._gh_comics_get_detail(id, option?:{source?:string,is_cache?:boolean})`
## 接口 - `window._gh_comics_get_pages(id, option?:{source?:string,is_cache?:boolean})`
## 接口 - `window._gh_comics_get_image(id, option?:{source?:string,is_cache?:boolean})`
