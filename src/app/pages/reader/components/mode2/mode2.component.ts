...
@Component({
  selector: 'app-mode2',
  templateUrl: './mode2.component.html',
  styleUrls: ['./mode2.component.scss']
})
export class Mode2Component {
  images = [];
  index = 0;
  total = 0;
  id = "";
  ...
  constructor(
    ...
    GamepadEvent.registerAreaEvent('reader_mode_2', {
      UP: () => {
        let node = document.querySelector("#mode2");
        for (let i = 1; i < 41; i++) {
          setTimeout(() => {
            node.scrollTop = node.scrollTop - 3;
          }, 10 * i)
        }
      },
      LEFT: () => {
  ...
  async init(id) {
    const container = document.getElementById("mode2")
