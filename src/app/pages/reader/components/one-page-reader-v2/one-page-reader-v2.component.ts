...
@Component({
  selector: 'app-one-page-reader-v2',
  templateUrl: './one-page-reader-v2.component.html',
  styleUrls: ['./one-page-reader-v2.component.scss']
})
export class OnePageReaderV2Component {
  swiper = null;
  ...
  async addNextSlide(chapter_id, list, index: number) {
    ...
    const getNextPages = async (list: Array<PagesItem>, index: number) => {
      ...
      if (obj?.primary?.width==obj?.secondary?.width && !this.is_1) {
        document.documentElement.style.setProperty('--double-page-reader-v2-width', `${(obj.primary.width / obj.primary.height) * window.innerHeight}px`);
        this.is_1 = true
      }
