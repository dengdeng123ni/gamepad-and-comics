...
@Component({
  selector: 'app-mode4',
  templateUrl: './mode4.component.html',
  styleUrls: ['./mode4.component.scss']
})
export class Mode4Component {
  ...
  constructor(
    ...
    GamepadEvent.registerAreaEvent('reader_mode_4', {
      "UP": () => {
    ...
    this.nextPage$ = this.current.nextPage().subscribe(() => {
      this.config.mode4.isPageTurnDirection ? this.next() : this.previous();
    })
    this.previousPage$ = this.current.previousPage().subscribe(() => {
      this.config.mode4.isPageTurnDirection ? this.previous() : this.next();
    })
  }
  ngOnDestroy() {
  ...
  swiperConfig: SwiperOptions = {
    ...
    direction: this.config.mode4.slidingDirection == "vertical" ? "vertical" : "horizontal",
  ...
  async change(index) {
    ...
    if (this.config.mode4.isPageTurnDirection) {
  ...
  init = () => {
    this.oBox = document.querySelector('#mode4')
