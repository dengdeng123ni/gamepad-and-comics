...
export class ZipService {
  ...
  async images(arr1: Array<{ path: string, src: string }>){
    const { arr, paths } = await this.getImageBlob2(arr1);
    const json = new Blob([JSON.stringify(paths)], { type: 'application/json' })
    var zip = new JSZip();
    arr.forEach(x => {
      zip.file(x.path, x.blob)
    })
    zip.file(`images/index.json`, json)
    zip.generateAsync({ type: "blob" }).then(function (content) {
      saveAs(content, "images.zip");
    })
  }
  async getImageBlob2(images: Array<{ path: string, src: string }>) {
    let arr = [];
    let paths = [];
    for (let i = 0; i < images.length; i++) {
      const x = images[i];
      ...
      const srcs = blob.type.split("/");
      const path = `${x.path}.${srcs.at(-1)}`;
      arr.push({ path: `images/${path}`, blob: blob })
      paths.push(path);
    }
    return { arr, paths: { v2: paths } }
  }
