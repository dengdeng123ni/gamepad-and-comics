...
export class NovelsListComponent {
    ...
    id$.subscribe(async (params) => {
      ...
      if (data) {
        ...
        if (this.type == "multipy") {
          ...
          this.list = await this.NovelsList.Events[this.id].Init({ page_num: 1, page_size: data.list.length });
  ...
  async on_list($event: MouseEvent) {
    ...
    if (node.getAttribute("id") == 'comics_list') {
      ...
      const getTargetNode = (node: HTMLElement): HTMLElement => {
        if (node.getAttribute("region") == "novels_item") {
          return node
        } else {
      ...
      if (this.data.is_edit || this._ctrl) {
        ...
        if (this.data.config.click_type == 1) {
          ...
          this.routerReader(this.source, data.id)
        }
  ...
  async on(index) {
    ...
    this.list = await this.NovelsList.init(this.key, { page_num: this.page_num });
  }
  ...
  async put() {
    ...
    return this.NovelsList._data[this.id] = obj
  }

  async get(id) {
    const res = this.NovelsList._data[id];
    if (res) {
      return res
    } else {
      return null
  ...
  async add_pages() {
    ...
    this.page_num++;
    const list = await this.NovelsList.add(this.key, { page_num: this.page_num, page_size: this.page_size })
    if (list.length == 0) {
      this.page_num--;
      return
    }
