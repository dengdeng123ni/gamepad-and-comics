...
registerGlobalEvent
  init(){
    this.GamepadEvent.registerAreaEvent("handel_toolabr_center", {
      "B": () => this.close(),
      "A": () => {
        this.close();
        (document.querySelector(`[_id=handel_toolabr_center_${this.current}]`) as any).click();
      ...
      LEFT_TRIGGER:() => {
        this.GamepadController.setCurrentTargetId("handel_toolabr_left_1")
        document.querySelector(".inner-triangle").classList.add("opacity-0")
      },
      RIGHT_TRIGGER:() => {
        this.GamepadController.setCurrentTargetId("handel_toolabr_right_1")
        document.querySelector(".inner-triangle").classList.add("opacity-0")
      }
    })
    this.GamepadEvent.registerAreaEvent("handel_toolabr_left", {
      "B": () => this.close(),
      "UP": () => {
    ...
    this.GamepadEvent.registerAreaEvent("handel_toolabr_right", {
      "B": () => this.close(),
      "UP": () => {
        this.GamepadController.setCurrentRegionTarget("UP");
      },
    ...
    this.up$=this.GamepadInput.up().subscribe(e => {
      if (e == "NO_DIRECTION" && this.opened) {
        if(this.GamepadController.current.region!="handel_toolabr_center") return
