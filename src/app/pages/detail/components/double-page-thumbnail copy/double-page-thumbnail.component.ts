...
export class DoublePageThumbnailComponent {
  ...
  async init(id, index) {
    let _list=[];
    for (let index = 0; index < this.current.comics.chapters.length; index++) {
      const x = this.current.comics.chapters[index];
      const images=x.images.map(x => ({ id: x.id, width: x.width, height: x.height, src: x.small }));
      const double_list=await this.images.getPageDouble(images, { isFirstPageCover: this.current.comics.isFirstPageCover, pageOrder: this.current.comics.pageOrder })
      _list.push({...x,double_list})
