...
import { DbComicsControllerService, HistoryService, IndexdbControllerService, NotifyService, PagesItem, RoutingControllerService } from 'src/app/library/public-api';
...
export class CurrentService {
  ...
  async _init(source, comics_id: string) {
    ...
    const _res = await Promise.all([this.DbComicsController.getDetail(comics_id, { source: source }), this._getWebDbComicsConfig(comics_id)])
    if(_res[0]===null){
      this.Notify.messageBox('当前数据访问失败,已自动返回页面', '', { panelClass: "_chapter_prompt", duration: 1000, horizontalPosition: 'center', verticalPosition: 'top', });
  ...
  async _getComicsRead(comics_id: string) {
    const res: any = await this.webDb.getByKey("last_read_comics", comics_id.toString())
    if (res) {
      return res
    } else {
