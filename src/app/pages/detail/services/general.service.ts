...
export class GeneralService {
  ...
  constructor(public current: CurrentDetailService, private db: NgxIndexedDBService,) {
  ...
  async createImage(src): Promise<any> {
    if (!src) {
      return {
        width: 0,
        height: 0
    ...
    return await createImageBitmap(await fetch(src).then((r) => r.blob()))
  }
