...
export class GamepadInputService {
  ...
  constructor(public Gamepad: GamepadService, private ngZone: NgZone) {
    ...
    this.Gamepad.connect().subscribe(() => {
      ...
      this.Gamepad.on('up0').subscribe(() => { this.press$.next("LEFT_ANALOG_UP") });
      this.Gamepad.on('down0').subscribe(() => { this.press$.next("LEFT_ANALOG_DOWN") });
      this.Gamepad.on('right0').subscribe(() => { this.press$.next("LEFT_ANALOG_RIGHT") });
      this.Gamepad.on('left0').subscribe(() => { this.press$.next("LEFT_ANALOG_LEFT") });

      this.Gamepad.on('up1').subscribe(() => { this.press$.next("RIGHT_ANALOG_UP") });
      this.Gamepad.on('down1').subscribe(() => { this.press$.next("RIGHT_ANALOG_UP") });
      this.Gamepad.on('right1').subscribe(() => { this.press$.next("RIGHT_ANALOG_UP") });
      this.Gamepad.on('left1').subscribe(() => { this.press$.next("RIGHT_ANALOG_UP") });

      this.Gamepad.on('button0').subscribe(() => { this.press$.next("A") });
      this.Gamepad.on('button1').subscribe(() => { this.press$.next("B") });
      this.Gamepad.on('button2').subscribe(() => { this.press$.next("X") });
      this.Gamepad.on('button3').subscribe(() => { this.press$.next("Y") });

      this.Gamepad.on('button4').subscribe(() => { this.press$.next("LEFT_TRIGGER") });
      this.Gamepad.on('button5').subscribe(() => { this.press$.next("RIGHT_TRIGGER") });
      this.Gamepad.on('button6').subscribe(() => { this.press$.next("LEFT_BUMPER") });
      this.Gamepad.on('button7').subscribe(() => { this.press$.next("RIGHT_BUMPER") });

      this.Gamepad.on('button8').subscribe(() => { this.press$.next("SELECT") });
      this.Gamepad.on('button9').subscribe(() => { this.press$.next("START") });

      this.Gamepad.on('button10').subscribe(() => { this.press$.next("LEFT_ANALOG_PRESS") });
      this.Gamepad.on('button11').subscribe(() => { this.press$.next("RIGHT_ANALOG_PRESS") });

      this.Gamepad.on('button12').subscribe(() => { this.press$.next("DPAD_UP") });
      this.Gamepad.on('button13').subscribe(() => { this.press$.next("DPAD_DOWN") });
      this.Gamepad.on('button14').subscribe(() => { this.press$.next("DPAD_LEFT") });
      this.Gamepad.on('button15').subscribe(() => { this.press$.next("DPAD_RIGHT") });

      this.Gamepad.on('button16').subscribe(() => { this.press$.next("CENTER") });
